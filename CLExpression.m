...
-(id) evaluate:(id) anObject
{
  ...
  case CLOperandOperator:
    aValue = [anObject valueForExpression:left];
    break;
    
  case CLLogicalNotOperator:
    aValue = [left evaluate:anObject];
    if ([aValue isKindOfClass:[CLNumber class]])
  ...
  case CLLogicalOrOperator:
    ...
    aValue = [CLNumber numberWithBool:val];
    if (![aValue boolValue]) {
      aValue = [right evaluate:anObject];
      if ([aValue isKindOfClass:[CLNumber class]])
	val = [aValue boolValue];
      else
	val = !!aValue;
