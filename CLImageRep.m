...
CLString *CLImageTypes[] = {@"", @"gif", @"jpg", @"png", nil};
CLString *CLImageDirs[] = {@"", @"Images/", @"images/", NULL};
...
-(void) scaleImage:(CLSize) aSize blend:(BOOL) flag
{
  ...
  unsigned int wantx, wanty, newx, newy;
  ...
  wantx = aSize.width;
  wanty = aSize.height;
  newx = aRep->width * xpercent;
  newy = aRep->height * ypercent;

  /* FIXME - hack to compensate for floating point error because I'm
     too lazy to rewrite piclib and all the other image related
     stuff. */
  if (newx < wantx)
    xpercent = (aSize.width + 1) / aRep->width;
  if (newy < wanty)
    ypercent = (aSize.height + 1) / aRep->height;

  scaleImage(xpercent, ypercent, flag, aRep);
  [self clearPath];
...
-(void) zoomImage:(CLSize) mSize blend:(BOOL) blend slideTo:(CLString *) direction
{
  ...
  if (cSize.width != nSize.width || cSize.height != nSize.height) {
    ...
    if (direction) {
      if (towlower([direction characterAtIndex:0]) == 'r')
	aRect.origin.x = cSize.width - nSize.width;
      else if (towlower([direction characterAtIndex:0]) == 'l')
	aRect.origin.x = 0;
    }

    aRect.origin.y = (cSize.height - nSize.height) / 2;
    if (direction) {
      if (towlower([direction characterAtIndex:0]) == 'b')
	aRect.origin.y = cSize.height - nSize.height;
      else if (towlower([direction characterAtIndex:0]) == 't')
	aRect.origin.y = 0;
    }
...
-(void) composite:(CLImageRep *) aRep from:(CLRect) aRect to:(CLPoint) aPoint
{
  ...
  for (y = 0; y < aRect.size.height; y++) {
    for (x = 0; x < aRect.size.width; x++) {
      as = ad = 255;
      if (src->photoInterp & ALPHAMASK)
	as = getval(x + aRect.origin.x, y + aRect.origin.y, scol, src);
      if (dest->photoInterp & ALPHAMASK)
	ad = getval(x + aPoint.x, y + aPoint.y, dcol, dest);

      if (scol == dcol) {
	for (col = 0; col < scol; col++) {
	  vs = getval(x + aRect.origin.x, y + aRect.origin.y, col, src);
	  vd = getval(x + aPoint.x, y + aPoint.y, col, dest);

	  vd = as * vs + ad * vd - as * ad * vd;
	  putval(x + aPoint.x, y + aPoint.y, vd, col, dest);
	}


	if (dest->photoInterp & ALPHAMASK) {
	  ad = as + ad - ad * as;
	  putval(x + aPoint.x, y + aPoint.y, ad, dcol, dest);
	}	
      }
      else if (scol < dcol) {
	vs = getval(x + aRect.origin.x, y + aRect.origin.y, col, src);
