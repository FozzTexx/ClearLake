...
-(void) last:(id) sender
{
  ...
  aString = [attributes objectForCaseInsensitiveString:@"CL_DATASOURCE"];
  aView = [[sender page] datasourceForBinding:aString];
...
-(CLBlock *) templateNamed:(CLString *) aString in:(id) aValue
{
  ...
  if ([aValue isKindOfClass:[CLBlock class]] &&
      [[[aValue attributes] objectForCaseInsensitiveString:@"name"]
	isEqualToString:aString])
    return aValue;

  if ([aValue respondsTo:@selector(value)] &&
      [(anObject = [aValue value]) isKindOfClass:[CLArray class]] &&
...
-(void) setDatasource:(id) aDatasource for:(id) anObject
{
  ...
  if ([anObject respondsTo:@selector(value)] &&
      [(anObject = [anObject value]) isKindOfClass:[CLArray class]])
...
-(BOOL) replaceTemplates:(id) aValue with:(id) anObject
{
  ...
  if ([aValue isKindOfClass:[CLArray class]]) {
    for (i = k = nf = 0, j = [aValue count]; i < j && nf < 2; i++) {
      ...
      if ([aBlock isKindOfClass:[CLBlock class]]) {
  ...
  else if ([aValue respondsTo:@selector(value)] &&
      [(aValue = [aValue value]) isKindOfClass:[CLArray class]] &&
...
-(void) writeHTML:(CLStream *) stream
{
  ...
  aDict = [[self datasource] extraQuery];
  ...
  for (i = start; i < end; i += maxItems) {
    ...
    [[aControl localQuery] addEntriesFromDictionary:aDict];
    [aControl addObject:aString];
    [aControl setPage:page];
    
    if (i < aRange.location || i >= CLMaxRange(aRange)) {
      if ((aBlock = [self templateNamed:@"cl_active" in:value])) {
	aBlock = [aBlock copy];
	...
	[mArray addObject:aControl];
      ...
      if ((aBlock = [self templateNamed:@"cl_inactive" in:value])) {
	aBlock = [aBlock copy];
	[mArray addObject:aBlock];
	[self setDatasource:aControl for:aBlock];
      }
  ...
  if (value) {
    if ([value isKindOfClass:[CLArray class]])
      aValue = [[CLMutableArray alloc] initWithArray:value copyItems:YES];
    else
      aValue = [value copy];
  }
  else
