...
+(CLCategory *) categoryWithID:(int) anID
{
  ...
  aCat = [[CLCategory alloc] initFromObjectID:anID];
...
+(CLCategory *) categoryWithTitle:(CLString *) aString andParentID:(int) anID
{
  ...
  fullTable = [CLGenericRecord tableForClass:[self class]];
...
{
  if (![anObject isKindOf:[Category class]])
    return NO;

  if ([anObject objectID] == objectID)
    return YES;

  if (![title isEqualToString:[anObject title]])
    return NO;

  if ((!parent && ![anObject parent]) ||
      [parent isEqual:[anObject parent]])
    return YES;
