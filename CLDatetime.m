...
CLUInteger CLAbsoluteGregorianDay(CLUInteger day, CLUInteger month, CLUInteger year)
{
  if (month > 1)
    while (--month)
...
void CLGregorianDateFromAbsolute(CLTimeInterval abs, int *day, int *month, int *year)
{
  *year = abs / 366;
...
+(CLDatetime *) now
{
  ...
  aDate = [[[self alloc] init] autorelease];
  return aDate;
}
...
{
  return [[[self alloc] initWithString:description format:format timeZone:nil]
	   autorelease];
}
...
{
  return [[[self alloc] initWithString:description format:format timeZone:aZone]
	   autorelease];
}
...
{
  return [self initWithString:description format:nil timeZone:nil];
}

-(id) initWithString:(CLString *) description format:(CLString *) aFormat
{
  return [self initWithString:description format:aFormat timeZone:nil];
}
	    ...
	    timeZone:(CLTimeZone *) aZone
{
  ...
  [self initWithYear:tm.tm_year + 1900 month:tm.tm_mon + 1 day:tm.tm_mday
	hour:tm.tm_hour minute:tm.tm_min second:tm.tm_sec timeZone:aZone];
  [format release];
  format = [aFormat copy];
...
-(CLString *) propertyList
{
  ...
  return [CLString stringWithFormat:@"<*D%@>",
		   [self descriptionWithFormat:@"%Y-%m-%d %H:%M:%S +0000"
			 timeZone:[CLTimeZone UTCTimeZone]]];
}

-(CLString *) json
{
#if 0
  return [self descriptionWithFormat:@"%Y-%m-%dT%H:%M:%SZ" timeZone:[CLTimeZone UTCTimeZone]];
#else
...
{
  return [self descriptionWithFormat:aFormat timeZone:nil];
}

-(CLString *) descriptionWithTimeZone:(CLTimeZone *) aZone
{
  return [self descriptionWithFormat:format timeZone:aZone];
}

-(CLString *) descriptionWithFormat:(CLString *) aFormat timeZone:(CLTimeZone *) aZone
{
  struct tm tm;
  char *tbuf;
  size_t tsize;
