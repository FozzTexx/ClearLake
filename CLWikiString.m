...
@interface CLWikiString (PrivateMethods)
	     ...
	     intoDictionary:(CLMutableDictionary *) mDict
{
  ...
  while (indx < len) {
	...
	indx++;
      ...
      [mDict setObject:[CLNull null] forKey:key];
    [key release];
...
-(CLArray *) images
{
  return [self filterByClass:[CLWikiImage class]];
}
  
-(CLArray *) links
{
  return [self filterByClass:[CLWikiLink class]];
}

-(CLArray *) media
{
  return [self filterByClass:[CLWikiMedia class]];
}
