...
-(CLBucket *) bucketForKey:(id) aKey hash:(CLUInteger) aValue selector:(SEL) aSel
{
  ...
  aBucket = table[val];
  while (aBucket) {
    imp = objc_msg_lookup(aBucket->key, aSel);
    if (aBucket->key == aKey || ((BOOL (*) (id,SEL,id)) imp)(aBucket->key, aSel, aKey))
      return aBucket;
...
CLHashTable *CLHashTableAlloc(CLInteger size)
{
  return [[CLHashTable alloc] initWithSize:size];
}

void CLHashTableFree(CLHashTable *ht)
{
  [ht release];
}

void CLHashTableSetData(CLHashTable *ht, void *data, id aKey, CLUInteger hash)
{
  [ht setData:data forKey:aKey hash:hash];
  return;
}

void *CLHashTableDataForKey(CLHashTable *ht, id aKey, CLUInteger hash, SEL selector)
{
  return [ht dataForKey:aKey hash:hash selector:selector];
}

void *CLHashTableDataForIdenticalKey(CLHashTable *ht, id aKey, CLUInteger hash)
{
  return [ht dataForKeyIdenticalTo:aKey hash:hash];
}

id CLHashTableKeyForKey(CLHashTable *ht, id aKey, CLUInteger hash, SEL selector)
{
  return [ht keyForKey:aKey hash:hash selector:selector];
}

void *CLHashTableRemoveDataForKey(CLHashTable *ht, id aKey, CLUInteger hash, SEL selector)
{
  return [ht removeDataForKey:aKey hash:hash selector:selector];
}

void *CLHashTableRemoveDataForIdenticalKey(CLHashTable *ht, id aKey, CLUInteger hash)
{
  return [ht removeDataForKeyIdenticalTo:aKey hash:hash];
}

void CLHashTableGetKeys(CLHashTable *ht, id *buf)
{
  [ht getKeys:buf];
  return;
}

void CLHashTableGetData(CLHashTable *ht, void **buf)
{
  [ht getData:buf];
  return;
}
